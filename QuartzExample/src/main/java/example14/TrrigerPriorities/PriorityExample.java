 ...
 * 
 */
 package example14.TrrigerPriorities;
...
public class PriorityExample {
  ...
  public void run() throws Exception {
    ...
    SchedulerFactory sf = new StdSchedulerFactory("example14/TrrigerPriorities/quartz_priority.properties");
    ...
    Trigger trigger1 = newTrigger().withIdentity("Priority1Trigger5SecondRepeat").startAt(startTime)
        .withSchedule(simpleSchedule().withRepeatCount(1).withIntervalInSeconds(5)).withPriority(15).forJob(job).build();
    ...
    Trigger trigger2 = newTrigger().withIdentity("Priority5Trigger10SecondRepeat").startAt(startTime)
        .withSchedule(simpleSchedule().withRepeatCount(1).withIntervalInSeconds(10)).withPriority(10).forJob(job).build();
    ...
    Trigger trigger3 = newTrigger().withIdentity("Priority10Trigger15SecondRepeat").startAt(startTime)
        .withSchedule(simpleSchedule().withRepeatCount(1).withIntervalInSeconds(15)).withPriority(5).forJob(job)
        .build();
    
    // Third trigger has priority 10, and will repeat after 15 seconds
    Trigger trigger4 = newTrigger().withIdentity("Priority15Trigger15SecondRepeat").startAt(startTime)
        .withSchedule(simpleSchedule().withRepeatCount(1).withIntervalInSeconds(15)).withPriority(1).forJob(job)
        .build();
